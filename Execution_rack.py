...
from UR_tasks_seda_camera import URTasks as URT

# python3 Execution_rack.py --rack_number 1 --rack_number 2 --rack_number 3 --rack_number 4 --loops 1
    ...
    for loop_index in range(args.loops):
        for rack_number in args.rack_number:
            ...
            for vial_info in rack_info:
                ...
                time.sleep(1)
                robot.open_laser()
                robot.capture_image(vial_number=vial_number, rack_number=rack_number, sample_name=sample_name, loop_iteration=loop_index) #before robot.capture_image_focus_adjustment now capture_image
                time.sleep(5)
                robot.close_laser()
                robot.open_lid()
                time.sleep(0.5)
...
def test():
    ...
    # robot.activate_gripper()
    # robot.open_gripper_width(200)
    # robot.test_pick_place_rack_from_shelf(5)
    robot.go_to_middle()
    robot.go_avoid()
    robot.go_home()
    # for rack_number in args.rack_number:
...
def camera_test():
    args = parse_arguments()
    robot = URT(ip="192.168.56.6", port=30003)
    robot.capture_image(1, 1, "test", 1)
