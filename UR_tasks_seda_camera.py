...
class URTasks:
    def __init__(self, ip, port):
        ...
        3: [-0.60341506, 0.07396000, 0.37652685, -2.33738635, -0.00460993, -0.00568369],
        4: [-0.47208899, 0.07621485, 0.37872479, -2.36331031, -0.01361378, -0.01610734],
        5: [-0.60423679, 0.07493089, 0.22396544, -2.36329816, -0.01365947, -0.01627141],
        6: [-0.46923238, 0.07654163, 0.22708680, -2.37788843, -0.01384540, -0.01624501],
        # Add more pre_drop positions for other vials here
        }

        # backup before changing resin holders: 28/10/2024
        # 1: [-0.60188087, 0.12404068, 0.54251319, -2.36690799, -0.03411182, -0.00540021],
        # 2: [-0.46929819, 0.12265601, 0.54236534, -2.36690056, -0.03416486, -0.00546280],
        ...
        3: [-0.60341506, 0.07396000, 0.39454858, -2.33738635, -0.00460993, -0.00568369],
        4: [-0.47208899, 0.07621485, 0.39454858, -2.36331031, -0.01361378, -0.01610734],
        5: [-0.60423679, 0.07493089, 0.23838443, -2.36329816, -0.01365947, -0.01627141],
        6: [-0.46923238, 0.07654163, 0.23838443, -2.37788843, -0.01384540, -0.01624501],
        # Add more pre_drop positions for other vials here
        }
    ...
    def load_vial_to_box(self):
        ...
        pre_insertion = [0.40678861, 0.00573256, 0.25936172, 2.22268577, -2.22012215, -0.00811071]
        insertion = [0.40678619, 0.00573888, 0.21342104, 2.22269176, -2.22012848, -0.00812468]
        self.go_to_prebox()
        self.go_to_abovebox()
    ...
    def unload_vial_from_box(self):
        ...
        pre_pick = [0.40678861, 0.00573256, 0.25936172, 2.22268577, -2.22012215, -0.00811071]
        pick = [0.40678619, 0.00573888, 0.21342104, 2.22269176, -2.22012848, -0.00812468]
    ...
    def test_pick_place_rack_from_shelf(self, i):
        logging.info(f"Starting picking rack{i} from the shelf")
        pre_pick = self.rack_pre_picking_shelf[i]
        pick = self.rack_picking_positions_shelf[i]
        transfer = self.rack_transfer[i]
    ...
    def capture_image(self, vial_number, rack_number, sample_name, loop_iteration):
        """Captures an image from OBS Virtual Camera (video4) and saves it using the exact filename format from the original code."""

        # Set the OBS Virtual Camera index
        camera_index = 6  # Hardcoded since we confirmed OBS is at /dev/video6

        # # Construct the correct rack CSV filename (e.g., rack_1.csv, rack_2.csv, etc.)
        # rack_csv_path = f"rack_{rack_number}.csv"

        # try:
        #     # Read CSV file to get the correct sample name
        #     rack_df = pd.read_csv(rack_csv_path)

        #     # Find the corresponding sample name from CSV
        #     sample_name_from_csv = rack_df.loc[
        #         (rack_df['Rack'] == rack_number) & (rack_df['Vial'] == vial_number),
        #         'Sample Name'
        #     ].values

        #     # Use the sample name from the CSV if available, otherwise use provided `sample_name`
        #     if len(sample_name_from_csv) > 0:
        #         sample_name = sample_name_from_csv[0]

        # except FileNotFoundError:
        #     print(f"Error: Rack CSV file not found: {rack_csv_path}")

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

        # Construct the image filename (same format as your previous code)
        # filename = f'image/rack_{rack_number}_vial_{vial_number}_{sample_name.replace(" ", "_")}_loop{loop_iteration}.jpg'

        filename = f'image/rack_{rack_number}_vial_{vial_number}_{sample_name.replace(" ", "_")}_loop{loop_iteration}_{timestamp}.jpg'

        # Open OBS Virtual Camera (using /dev/video4)
        cap = cv2.VideoCapture(camera_index)

        if not cap.isOpened():
            print("Error: Could not open OBS Virtual Camera at /dev/video6.")
            return
        ...
        ret, frame = cap.read()
        if not ret:
            print("Error: Could not read frame from OBS Virtual Camera.")
            cap.release()
            return
